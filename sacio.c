...
/* for fortran--write evenly-spaced data with az and user1 set*/
void    wrtsac1_(const char *name, float *dt, int *ns, float *b0,float *tp,float *ts, float *dist, float *az, float *user1, const float *ar) {
  SACHEAD hd;
  hd = sachdr(*dt,*ns,*b0);
  hd.gcarc = *dist;
  hd.az = *az;
